...
<body>
    ...
    <div class="container">
        <ul class="nav-links">
            <li><a href="../php/dataKaryawan.php">Data Karyawan</a></li>
        ...
        if (mysqli_num_rows($result) > 0) {
            while ($row = mysqli_fetch_assoc($result)) {
                ...
                echo "<button class='hapus-lowongan' data-id='{$row['id']}'>Hapus Lowongan</button>";
            ...
            echo "<p style='margin-top: 10px;'>Maaf, tidak ada lowongan pekerjaan yang tersedia saat ini.</p>";
        }
        ?>
    </div>
    ...
    <script>
        ...
        var hapusLowonganButtons = document.querySelectorAll('.hapus-lowongan');
        hapusLowonganButtons.forEach(function(button) {
            button.addEventListener('click', function() {
                var idLowongan = this.getAttribute('data-id');
                if (confirm('Apakah Anda yakin ingin menghapus lowongan ini?')) {
                    // Kirim permintaan ke server untuk menghapus lowongan
                    var xhr = new XMLHttpRequest();
                    xhr.open("POST", "hapusLowongan.php", true);
                    xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
                    xhr.onreadystatechange = function() {
                        if (xhr.readyState === 4 && xhr.status === 200) {
                            var response = xhr.responseText;
                            alert(response);
                            // Reload halaman setelah menghapus lowongan
                            window.location.reload();
                        }
                    };
                    xhr.send("idLowongan=" + idLowongan);
                }
            });
        });
    </script>
